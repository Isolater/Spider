...
from items import DoubanmoviespiderItem
...
class MovieSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(allow=r'/subject/\d+/($|\?\w+)'),
             callback='pares_movie', follow=True),
    )
    ...
    def parse_movie(self, response):
        ...
        self.get_channel(response, item)
    ...
    def get_year(self, response, item):
        year = response.xpath('//*[@id="content"]/h1/span[2]/text()').re(NUM_RE)
    ...
    def get_actors(self, respose, item):
        actors = respose.xpath("//a[@rel='v:starring']/text()").extract()
    ...
    def get_stars(self, response, item):
        ...
        xpath = '//*[@id="interest_sectl"]/div[1]/div[2]/div/div[1]/@class'
        stars = float(response.xpath(xpath)[-2:] / 10)
        item['stars'] = stars

    def get_channel(self, response, item):
